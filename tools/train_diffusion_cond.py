...
def train(args):
    ...
    model = DiffusionModelUNet(
        ...
        cross_attention_dim=8  # label(3) + normalized demo(5)
    ).to(DEVICE)
    ...
    perceptual_loss_fn = PerceptualLoss(spatial_dims=2, network_type="resnet50", pretrained=True).to(DEVICE)
    ...
    for epoch in range(args.n_epochs):
        ...
        for step, batch in pbar:
            ...
            demo = batch['demo'].to(DEVICE)  # [B, 5]
            num_classes = 3

            target_label = torch.stack([torch.tensor(np.random.choice([c for c in range(num_classes) if c != l.item()])) for l in label]).to(DEVICE)
            onehot_label = F.one_hot(target_label, num_classes=num_classes).float()  # [B, 3]
            condition_vec = torch.cat([onehot_label, demo], dim=-1).unsqueeze(1)  # [B, 1, 8]
            ...
            for i in range(n_train_steps):
                ...
                for b in range(x_t.shape[0]):
                    x_t_b, _ = scheduler.step(noise_pred[b:b+1], t[b].item(), x_t[b:b+1])
            ...
            img_lo, img_hi = percentile(image, 10.0), percentile(image, 90.0)
            pred_lo, pred_hi = percentile(x0_norm, 10.0), percentile(x0_norm, 90.0)
            ...
            loss_cls = F.cross_entropy(classifier(cf_image), target_label)
            cf_map = cf_image - image
            loss_cf_map = cf_l1_weight * torch.mean(torch.abs(cf_map)) + cf_l2_weight * torch.mean(cf_map ** 2)
            loss_perc = perceptual_loss_fn(cf_image.repeat(1,3,1,1), image.repeat(1,3,1,1))
        ...
        if avg_loss < best_loss - 1e-4:
            ...
            torch.save(model.state_dict(), os.path.join(args.output_dir, "unet_best_ckpt_cond.pth"))
        else:
            wait += 1
            if wait >= patience:
                torch.save(model.state_dict(), os.path.join(args.output_dir, "unet_ckpt_cond.pth"))
                break
        torch.save(model.state_dict(), os.path.join(args.output_dir, "unet_ckpt_cond.pth"))
